...
export const SecurityActivityCenter: React.FC = () => {
  ...
  const getActionBadge = (action: string) => {
    ...
    return 'bg-zinc-500/10 text-slate-600 dark:text-zinc-400 border-slate-100 dark:border-white/5';
  };
  ...
  const renderPayloadDiff = (oldVal: any, newVal: any) => {
    if (!oldVal && !newVal) return <span className="text-slate-500 dark:text-zinc-500 italic">No values modified.</span>;
      ...
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
          <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3">
            <span className="text-[10px] text-slate-500 dark:text-zinc-500 block uppercase font-bold tracking-wider mb-1">Before</span>
            <span className="text-red-400 break-all">{JSON.stringify(oldVal) || 'undefined'}</span>
          </div>
          <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-3">
            <span className="text-[10px] text-slate-500 dark:text-zinc-500 block uppercase font-bold tracking-wider mb-1">After</span>
    ...
    if (changedFields.length === 0) {
      return <span className="text-slate-500 dark:text-zinc-500 italic text-[11px]">No fields modified in audit details.</span>;
    }

    return (
      <div className="border border-slate-100 dark:border-white/5 rounded-xl overflow-hidden bg-zinc-950/40 text-xs">
        <div className="grid grid-cols-3 gap-2 px-4 py-2 border-b border-slate-100 dark:border-white/5 bg-white/2 text-[10px] text-slate-600 dark:text-zinc-400 font-bold uppercase tracking-wider">
          <div>Field Key</div>
          ...
          {changedFields.map(key => {
            ...
            return (
              <div key={key} className="grid grid-cols-3 gap-2 px-4 py-3 items-start hover:bg-white/1 transition-all">
                <span className="text-slate-700 dark:text-zinc-300 font-semibold truncate" title={key}>{key}</span>
                <span className="text-red-400/90 break-all pr-2">
                  {beforeValue === undefined ? (
                    <span className="text-zinc-600 font-sans italic text-[10px]">undefined</span>
                  ) : beforeValue === null ? (
                    <span className="text-slate-500 dark:text-zinc-500 font-sans italic text-[10px]">null</span>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-border bg-card/40 glow-primary relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Security Activity Center</h3>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              className="bg-white dark:bg-zinc-900/60 border border-border/80 rounded-xl px-4 py-2 text-xs font-light text-white focus:outline-none focus:border-blue-500/50 transition-all cursor-pointer"
            >
              ...
              {logs.map((log) => {
                ...
                return (
                  <div key={log.id} className="relative group animate-scale-in">
                    ...
                    {/* Timeline card panel */}
                    <div className="glass-panel border border-slate-200/50 dark:border-border/50 hover:border-blue-500/20 bg-zinc-900/15 rounded-xl p-4 transition-all">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        {/* Event Details */}
                        <div className="flex items-start space-x-3">
                          {/* User Avatar */}
                          <div className="w-8.5 h-8.5 rounded-lg bg-slate-100/60 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 flex items-center justify-center font-bold text-xs text-slate-700 dark:text-zinc-300 shrink-0">
                            {initials}
                          </div>
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-[10px] text-slate-500 dark:text-zinc-500 font-light">
                            ...
                            <div className="flex items-center space-x-4 mt-1.5 text-[11px] text-slate-600 dark:text-zinc-400 font-light">
                              ...
                              <span className="font-mono text-[9px] text-slate-500 dark:text-zinc-500 truncate max-w-[150px]" title={`Record ID: ${log.recordId}`}>
                                ID: {log.recordId}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-row sm:flex-col sm:items-end justify-between sm:justify-start gap-2 shrink-0 border-t sm:border-t-0 border-slate-100 dark:border-white/5 pt-2 sm:pt-0">
                          ...
                          <span className="text-[9px] text-zinc-600 font-mono flex items-center space-x-1">
                            <Server className="w-3 h-3 text-slate-800 dark:text-zinc-300" />
                      ...
                      {hasPayload && (
                        <div className="mt-3.5 pt-3.5 border-t border-slate-100 dark:border-white/5 flex flex-col space-y-3">
                          <button
