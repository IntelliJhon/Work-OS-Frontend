...
export const ProjectSprints: React.FC = () => {
  ...
  const [newSubtaskRemarks, setNewSubtaskRemarks] = useState('');
  const [newSubtaskFiles, setNewSubtaskFiles] = useState<SubTaskFile[]>([]);
  ...
  // Draft state for Deliverable Detail drawer (batched save)
  const [draftName, setDraftName] = useState('');
  const [draftDescription, setDraftDescription] = useState('');
  const [draftAssignee, setDraftAssignee] = useState('');
  const [draftPriority, setDraftPriority] = useState<'low'|'medium'|'high'|'critical'>('medium');
  const [draftStartDate, setDraftStartDate] = useState('');
  const [draftDueDate, setDraftDueDate] = useState('');
  const [draftStatus, setDraftStatus] = useState('');
  const [drawerIsDirty, setDrawerIsDirty] = useState(false);
  const [drawerIsSaving, setDrawerIsSaving] = useState(false);
  ...
  const handleUploadSubtaskFilesDuringCreation = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0 || !activeTaskId) return;
    setIsUploadingSubtaskFile(true);
    try {
      const res = await uploadsApi.upload('TASK', activeTaskId, Array.from(files));
      const uploaded: SubTaskFile[] = res.uploads.map((u: any) => ({
        id: u.id,
        name: u.originalName,
        publicUrl: u.publicUrl,
      }));
      setNewSubtaskFiles((prev) => [...prev, ...uploaded]);
    } catch (err) {
      console.error('Failed to upload files during subtask creation', err);
  ...
  const handleAddSubtask = (taskId: string) => {
    ...
    const newSub: SubTask = {
      ...
      assigneeId: members.find(m => `${m.firstName || ''} ${m.lastName || ''}`.trim() === newSubtaskAssignee.trim())?.id,
      remarks: newSubtaskRemarks.trim() || undefined,
      files: newSubtaskFiles.length > 0 ? newSubtaskFiles : undefined,
    };
  ...
  const handleDownloadFile = async (fileId: string, publicUrl?: string) => {
    if (publicUrl) {
  ...
  useEffect(() => {
    if (activeTask) {
      ...
      setDraftAssignee(activeTask.assignee || 'unassigned@acme.com');
      setDraftPriority((activeTask.priority as any) || 'medium');
      setDraftStartDate(activeTask.startDate ? activeTask.startDate.substring(0, 10) : '');
      setDraftDueDate(activeTask.dueDate ? activeTask.dueDate.substring(0, 10) : '');
      setDraftStatus(activeTask.status || 'to_do');
      setDrawerIsDirty(false);
      setDrawerSaveSuccess(false);

      // Reset subtask form states
      setNewSubtaskTitle('');
      setNewSubtaskPriority('medium');
      setNewSubtaskAssignee('');
      setNewSubtaskRemarks('');
  ...
  const markDrawerDirty = () => { setDrawerIsDirty(true); setDrawerSaveSuccess(false); };

  const handleSaveDeliverable = async () => {
    if (!activeTask || !canUpdate || drawerIsSaving) return;
    setDrawerIsSaving(true);
    try {
      const timeEstimateParsed = localTimeEstimate === '' ? null : Math.floor(Number(localTimeEstimate));
      await handleUpdateTaskDetail(activeTask.id, {
        name: draftName.trim() || activeTask.name,
        description: draftDescription.trim() || undefined,
        assignee: draftAssignee,
        priority: draftPriority,
        startDate: draftStartDate || undefined,
        dueDate: draftDueDate || undefined,
        timeEstimate: isNaN(timeEstimateParsed as any) ? activeTask.timeEstimate : timeEstimateParsed,
      } as any);
      // If status changed, update separately (status triggers completedAt)
      if (draftStatus !== activeTask.status) {
        await handleToggleTaskStatus(activeTask.id, draftStatus as any);
      }
      setDrawerIsDirty(false);
      setDrawerSaveSuccess(true);
      setTimeout(() => setDrawerSaveSuccess(false), 2500);
    } catch (err) {
      console.error('Failed to save deliverable', err);
    } finally {
      setDrawerIsSaving(false);
    }
  };
      ...
      {activeTask && createPortal(
        <>
          ...
          <div className="fixed top-0 right-0 h-screen w-[320px] md:w-[480px] bg-slate-50 dark:bg-zinc-950 border-l border-slate-200 dark:border-border z-50 shadow-2xl flex flex-col animate-slide-in-right">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  disabled={!canUpdate}
                  value={draftName}
                  onChange={(e) => { setDraftName(e.target.value); markDrawerDirty(); }}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={(e) => { setDraftDescription(e.target.value); markDrawerDirty(); }}
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    value={draftAssignee}
                    onChange={(e) => { setDraftAssignee(e.target.value); markDrawerDirty(); }}
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    disabled={!canEditFull}
                    value={draftPriority}
                    onChange={(e) => { setDraftPriority(e.target.value as any); markDrawerDirty(); }}
                    className="w-full bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500 disabled:opacity-75 cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <DatePickerInput
                    ...
                    disabled={!canEditFull}
                    onChange={(val) => { setDraftStartDate(val); markDrawerDirty(); }}
                ...
                <div className="space-y-1.5">
                  ...
                  <DatePickerInput
                    value={draftDueDate}
                    disabled={!canEditFull}
                    onChange={(val) => { setDraftDueDate(val); markDrawerDirty(); }}
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => { setLocalTimeEstimate(e.target.value); markDrawerDirty(); }}
                  className="w-full bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500 disabled:opacity-75"
              ...
              <div className="space-y-1.5">
                ...
                <select
                  disabled={!canUpdate}
                  value={draftStatus}
                  onChange={(e) => { setDraftStatus(e.target.value); markDrawerDirty(); }}
              ...
              <div className="space-y-4 border-t border-slate-200 dark:border-white/5 pt-4">
                ...
                {canUpdate && (
                  <div className="bg-slate-50 dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800/80 rounded-2xl p-4 space-y-3.5">
                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Subtask Title</label>
                      <input
                        type="text"
                        placeholder="e.g. Implement token refresh logic"
                        value={newSubtaskTitle}
                        onChange={(e) => setNewSubtaskTitle(e.target.value)}
                        className="w-full mt-1 bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500 transition-all font-medium"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e: any) => setNewSubtaskPriority(e.target.value)}
                          className="w-full mt-1 bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500 font-bold"
                        >
                          <option value="low">Low</option>
                          <option value="medium">Medium</option>
                          <option value="high">High</option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full mt-1 bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500"
                        >
                    ...
                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-500 tracking-wider font-bold">Remarks</label>
                      <textarea
                        placeholder="Add details, notes, or criteria here..."
                        value={newSubtaskRemarks}
                        onChange={(e) => setNewSubtaskRemarks(e.target.value)}
                        rows={2}
                        className="w-full mt-1 bg-white dark:bg-background border border-slate-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:border-purple-500 resize-none transition-all"
                      />
                    </div>

                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-500 tracking-wider block mb-1">Attached Media</label>
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="hidden"
                          multiple
                          onChange={handleUploadSubtaskFilesDuringCreation}
                        />
                        <button
                          type="button"
                          disabled={isUploadingSubtaskFile}
                          onClick={() => subtaskFileInputRef.current?.click()}
                          className="flex items-center space-x-1.5 px-3 py-2 rounded-xl border border-dashed border-purple-300 dark:border-purple-800 bg-white dark:bg-background text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition text-xs cursor-pointer font-medium disabled:opacity-40"
                        >
                        ...
                        {newSubtaskFiles.length > 0 && (
                          <span className="text-[10px] text-slate-500 font-medium">
                            {newSubtaskFiles.length} file(s) attached
                          </span>
                        )}
                      </div>
                      ...
                      {newSubtaskFiles.length > 0 && (
                        <div className="mt-2 space-y-1">
                          {newSubtaskFiles.map((file, i) => (
                            <div key={file.id} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-[10px]">
                              <span className="truncate max-w-[200px] text-slate-700 dark:text-zinc-300">{file.name}</span>
                              <button
                                type="button"
                                onClick={() => setNewSubtaskFiles(prev => prev.filter((_, idx) => idx !== i))}
                                className="text-slate-450 hover:text-red-400"
                              >
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-2 border-t border-slate-200 dark:border-white/5">
                      <button
                        onClick={() => {
                          setNewSubtaskTitle('');
                          setNewSubtaskRemarks('');
                        ...
                        className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-400 font-medium text-xs hover:bg-slate-100 dark:hover:bg-zinc-800 transition cursor-pointer"
                      >
                        Clear
                      </button>
                      <button
                        onClick={() => handleAddSubtask(activeTask.id)}
                        disabled={!newSubtaskTitle.trim() || isUploadingSubtaskFile}
                        className="px-4 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition disabled:opacity-50 cursor-pointer"
                      >
                        Add Subtask
                      </button>
                    </div>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-start space-x-2.5 cursor-pointer flex-1 min-w-0" onClick={() => { if (canUpdate) handleToggleSubtask(activeTask.id, sub.id); }}>
                              ...
                              <div className="flex flex-col flex-1 min-w-0">
                                <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                                  <span className={`text-xs ${sub.done ? 'line-through text-slate-500' : 'text-slate-800 dark:text-zinc-300'} font-bold`}>
                                    {sub.title}
                                  </span>
                                  {sub.priority && (
                                    <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded border leading-none ${
                                      sub.priority === 'critical' ? 'bg-red-50 text-red-600 border-red-200 dark:bg-red-950/20 dark:text-red-400 dark:border-red-900' :
                                      sub.priority === 'high' ? 'bg-orange-50 text-orange-600 border-orange-200 dark:bg-orange-950/20 dark:text-orange-400 dark:border-orange-900' :
                                      sub.priority === 'medium' ? 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900' :
                                ...
                                {/* Assignee & Remarks display */}
                                {(sub.assignee || sub.remarks) && (
                                  <div className="mt-1 text-[10px] text-slate-500 dark:text-zinc-400 space-y-0.5 font-medium pl-0.5">
                                    {sub.assignee && (
                                      <p>Assigned to: <span className="text-slate-700 dark:text-zinc-300 font-semibold">{sub.assignee}</span></p>
                                    )}
                                    {sub.remarks && (
                                      <p className="italic text-slate-500 dark:text-zinc-400 pl-1.5 border-l border-slate-200 dark:border-zinc-800">“{sub.remarks}”</p>
                                    )}
                                  </div>
                                )}

                                {/* Attachments uploaded during subtask creation */}
                                {sub.files && sub.files.length > 0 && (
                                  <div className="mt-1.5 flex flex-wrap gap-1">
                                    {sub.files.map((file) => (
                                      <div key={file.id} className="flex items-center space-x-1 px-1.5 py-0.5 rounded bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-[9px]">
                                        <Paperclip className="w-2.5 h-2.5 text-slate-450 shrink-0" />
                                        <a
                                          href={file.publicUrl}
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="truncate max-w-[120px] hover:text-purple-500 hover:underline text-slate-600 dark:text-zinc-400 font-medium"
                                          title={file.name}
                                          onClick={(e) => e.stopPropagation()} // Prevent completing the subtask when clicking the file link
                                        >
                                          {file.name}
                                        </a>
                                      </div>
            ...
            {/* ── Sticky Save Footer ── */}
            {canUpdate && (
              <div className={`border-t border-slate-200 dark:border-white/5 px-6 py-4 flex items-center justify-between shrink-0 transition-all duration-300 ${drawerIsDirty ? 'bg-purple-500/5' : 'bg-transparent'}`}>
                <span className={`text-[10px] font-medium transition-all duration-200 ${drawerIsDirty ? 'text-amber-400' : drawerSaveSuccess ? 'text-emerald-400' : 'text-slate-400'}`}>
                  {drawerIsDirty ? '● Unsaved changes' : drawerSaveSuccess ? '✓ Saved successfully' : 'No pending changes'}
                </span>
                <button
                  onClick={handleSaveDeliverable}
                  disabled={!drawerIsDirty || drawerIsSaving}
                  className={`flex items-center space-x-2 px-5 py-2 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer
                    ${drawerIsDirty
                      ...
                      : 'bg-slate-100 dark:bg-zinc-900 text-slate-400 cursor-not-allowed opacity-50'
                    }`}
                >
                  {drawerIsSaving ? (
                    <>
