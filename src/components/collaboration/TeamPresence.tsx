...
interface TeamPresenceProps {
  projectId: string;
  currentPage: 'workflow' | 'sprints' | 'gates';
}
  ...
  return (
    <div className="flex items-center space-x-4 glass-panel px-4 py-2.5 rounded-2xl border border-border shadow-md">
      ...
      <div className="flex -space-x-2.5 overflow-hidden">
        ...
        {user && (
          <div
            className="w-7 h-7 rounded-full border-2 border-background flex items-center justify-center text-[10px] font-extrabold text-white cursor-help relative group"
        ...
        {visibleCollaborators.map((c) => (
          <div
            key={c.userId}
            className="w-7 h-7 rounded-full border-2 border-background flex items-center justify-center text-[10px] font-extrabold text-white cursor-help relative group animate-scale-in"
          ...
          >
            ...
            <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 hidden group-hover:block z-50 w-48 p-2 bg-card/95 border border-border rounded-xl shadow-xl text-left pointer-events-none">
              <p className="text-xs font-bold text-white">{c.firstName} {c.lastName}</p>
              <p className="text-[9px] text-zinc-400">{c.email}</p>
      ...
      {visibleCollaborators.length > 0 && (
        <span className="text-[9px] text-zinc-500 font-extrabold uppercase tracking-wide pl-1">
      ...
      {Object.keys(typingUsers).length > 0 && (
        <div className="flex items-center space-x-1.5 pl-3 border-l border-white/5 animate-pulse">
