      ...
      <div className="w-full max-w-4xl h-[85vh] glass-panel-heavy rounded-2xl border border-border shadow-2xl overflow-hidden flex flex-col sm:flex-row animate-scale-in">
        ...
        <div className="flex-1 bg-black/40 border-r border-border flex items-center justify-center relative overflow-hidden min-h-[50vh] sm:min-h-0">
          {isImage ? (
            ...
            <div className="w-full h-full overflow-auto p-6 font-mono text-xs text-slate-700 dark:text-zinc-300 whitespace-pre leading-relaxed select-text text-left">
            ...
            <div className="w-full h-full overflow-auto p-8 text-left text-slate-700 dark:text-zinc-300 leading-relaxed select-text prose prose-invert max-w-none">
              <h1 className="text-xl font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-white/5 pb-2 mb-4"># {file.name}</h1>
              <p className="text-xs text-slate-600 dark:text-zinc-400 italic mb-4">Governance compliance document</p>
            ...
            <div className="text-center space-y-3">
              ...
              <p className="text-xs text-slate-600 dark:text-zinc-400 font-light italic">Preview not supported for this file format.</p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">Document Inspector</h4>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">Filename</label>
                <p className="text-sm font-bold text-slate-900 dark:text-white mt-1 break-all select-all">{file.name}</p>
              </div>

              <div className="grid grid-cols-2 gap-4 border-t border-b border-slate-100 dark:border-white/5 py-4">
                <div>
                  <label className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">File Size</label>
                  <p className="text-xs font-extrabold text-blue-400 mt-1 font-mono">{formatBytes(file.size || 1024 * 45)}</p>
                </div>
                <div>
                  <label className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">MIME Type</label>
                  <p className="text-xs font-semibold text-slate-700 dark:text-zinc-300 mt-1 truncate" title={file.mimeType || 'unknown'}>
                    {file.mimeType || 'binary/stream'}
                  </p>
                </div>
              </div>

              <div className="space-y-3.5 text-xs text-slate-600 dark:text-zinc-400 font-light">
                <div className="flex items-center space-x-2.5">
                  <User className="w-4 h-4 text-slate-500 dark:text-zinc-500 flex-shrink-0" />
                  <div>
                    <span className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">Uploaded By</span>
                    <span className="text-slate-900 dark:text-white font-medium">{file.uploadedBy || 'admin@acme.com'}</span>
                  </div>
                </div>

                <div className="flex items-center space-x-2.5">
                  <Calendar className="w-4 h-4 text-slate-500 dark:text-zinc-500 flex-shrink-0" />
                  <div>
                    <span className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">Uploaded Date</span>
                ...
                <div className="flex items-center space-x-2.5 border-t border-slate-100 dark:border-white/5 pt-3.5">
                  <Database className="w-4 h-4 text-slate-500 dark:text-zinc-500 flex-shrink-0" />
                  <div>
                    <span className="text-[9px] font-bold text-slate-500 dark:text-zinc-500 uppercase tracking-widest block">Ledger Reference</span>
                    <span className="font-mono text-[9px] text-slate-500 dark:text-zinc-500">aud-gate-sign-4cf9b8</span>
                  </div>
                </div>
              </div>
            </div>
